...
export async function crearReserva(input: CrearReservaInput) {
  ...
  const disponibles = await calcularDisponibilidad(fecha, servicioId, resolvedEmpresaId || undefined);
  ...
  const clienteRegistrado = await buscarOCrearCliente(cliente, resolvedEmpresaId || undefined);
