...
async function calcularDisponibilidad(fecha, servicioId, empresaId) {
    ...
    if (config?.diasAtencion && Array.isArray(config.diasAtencion) && config.diasAtencion.length > 0) {
        const atiendeHoy = config.diasAtencion.some((d) => normalizarTexto(d) === diaNombreNormalizado);
        if (!atiendeHoy) {
    ...
    let turnos = [];
    const personalizacion = (typeof config?.personalizacion === 'string'
        ? JSON.parse(config.personalizacion)
        : config?.personalizacion) || {};
    // Temporadas estacionales (ej. feb-oct vs nov-dic)
    if (personalizacion.horarioEstacional?.activo &&
        Array.isArray(personalizacion.horarioEstacional.temporadas)) {
        const temporada = personalizacion.horarioEstacional.temporadas.find((t) => Array.isArray(t.meses) && t.meses.includes(month));
