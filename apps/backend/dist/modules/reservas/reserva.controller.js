...
exports.reservaRouter.get('/disponibilidad', async (req, res) => {
    try {
        ...
        const horarios = await (0, disponibilidad_service_1.calcularDisponibilidad)(fecha, servicio_id, req.empresa?.id);
...
exports.reservaRouter.get('/reservas/cliente/mis-reservas', async (req, res) => {
    try {
        ...
        const where = { cliente: whereCliente };
        if (req.empresa?.id) {
...
exports.adminReservaRouter.get('/reservas', async (req, res) => {
    const { fecha, fecha_inicio, fecha_fin, cliente, servicio_id, empleada_id, estado, } = req.query;
    const where = {};
...
exports.adminReservaRouter.get('/reportes/ocupacion', async (req, res) => {
    const whereEmpresa = req.empresa?.id ? { empresaId: req.empresa.id } : {};
    const total = await database_1.prisma.reserva.count({ where: whereEmpresa });
    const porEstado = await database_1.prisma.reserva.groupBy({
        by: ['estado'],
        where: whereEmpresa,
        _count: true,
    });
